...
function criarVaga(empresa_id) {
  ...
  if (
    new Date().getTime() >
    new Date(document.querySelector('#expireDate').value).getTime()
  ) {
    ErrorMsg.push({
      Erro: 'Data inválida, a data final não pode ser anterior a hoje',
    });
  }
  for (let vl in importantFields) {
    if (
      !$(importantFields[vl]).val() ||
      $(importantFields[vl]).val().length == 0
    ) {
