export function toast(message) {
  for (let vl in message) {
    Toastify({
      ...
      style: {
        background:
          Object.keys(message[vl]) == 'Sucesso'
            ? 'linear-gradient(to right, #00b09b, #96c93d)'
            : 'linear-gradient(to bottom right, #FF4950, #FFC9DC)',
      },
    }).showToast();
  }
}
...
export function verificarUsuario() {
  ...
  if (user && EmpresaUser) {
    for (let index = 0; index < EmpresaUser.length; index++) {
      if (
        user.email == EmpresaUser[index].login &&
        user.senha == EmpresaUser[index].password
      ) {
...
export const reduzirTexto = (text, length) =>
...
export const pegarEmpresaUsuario = () => {
  return JSON.parse(localStorage.getItem('EmpresaUser'));
};
export const pegarUsuario = () => {
  return JSON.parse(localStorage.getItem('userValid'));
};

export function pegarEmpresas({ id = null, arrayIds = null }) {
  if (id) {
    return JSON.parse(localStorage.getItem('Empresas')).find(
      (obj) => obj.id === parseInt(id),
    );
  } else if (arrayIds) {
    return JSON.parse(localStorage.getItem('Empresas')).filter((obj) =>
...
export function pegarVagas({ id = null, arrayIds = null }) {
  ...
  if (id) {
    return JSON.parse(localStorage.getItem('Vagas')).find(
      (obj) =>
        obj.id === parseInt(id) && new Date(obj.data_exp).getTime() > now,
    );
  } else if (arrayIds) {
    return JSON.parse(localStorage.getItem('Vagas'))
      .filter(
        (obj) =>
          arrayIds.includes(obj.id) && new Date(obj.data_exp).getTime() > now,
      )
      .reverse();
  } else {
    return JSON.parse(localStorage.getItem('Vagas'))
      .filter((obj) => new Date(obj.data_exp).getTime() > now)
      .reverse();
  }
}
export function pegarHabilidades({ id = null, arrayIds = null }) {
  if (id) {
    return JSON.parse(localStorage.getItem('Habilidades')).find(
      (obj) => obj.id === parseInt(id),
    );
  } else if (arrayIds) {
    return JSON.parse(localStorage.getItem('Habilidades')).filter((obj) =>
      arrayIds.includes(obj.id),
    );
  } else {
...
export function pegarSenioridade({ id = null, arrayIds = null }) {
  if (id) {
    return JSON.parse(localStorage.getItem('Senioridade')).find(
      (obj) => obj.id === parseInt(id),
    );
  } else if (arrayIds) {
    return JSON.parse(localStorage.getItem('Senioridade')).filter((obj) =>
      arrayIds.includes(obj.id),
    );
  } else {
...
export function pegarCargo({ id = null, arrayIds = null }) {
  if (id) {
    return JSON.parse(localStorage.getItem('Cargos')).find(
      (obj) => obj.id === parseInt(id),
    );
  } else if (arrayIds) {
    return JSON.parse(localStorage.getItem('Cargos')).filter((obj) =>
      arrayIds.includes(obj.id),
    );
  } else {
...
export function pegarContato({ id = null, arrayIds = null }) {
  if (id) {
    return JSON.parse(localStorage.getItem('Contato')).find(
      (obj) => obj.id === parseInt(id),
    );
  } else if (arrayIds) {
    return JSON.parse(localStorage.getItem('Contato')).filter((obj) =>
      arrayIds.includes(obj.id),
    );
  } else {
...
export function gerarCardsVagas(
  vagas,
  maxVagas,
  ...
  for (let vl in vagas) {
    if (vl < maxVagas || !maxVagas) {
      ...
      let habilidades = JSON.parse(localStorage.getItem('Habilidades')).filter(
        (obj) => vagas[vl].hab_essencial_id.includes(obj.id),
      );
      let cargo = pegarCargo({ id: vagas[vl].cargo_id });
      let senioridade = pegarSenioridade({
        id: vagas[vl].cargo_senioridade_id,
      });
      div += `
        <div class='border row col-sm-${colSize['sm']} col-md-${
        colSize['md']
      } col-lg-${colSize['lg']} align-items-center bg-light mt-2'>
          <div class="ratio ratio-1x1 col-sm-4">
            <a href='./empresa.html?id=${vagas[vl].empresa_id}'>
              <img src="${
                empresa.logo_link
              }" class="img-fluid rounded align-items-center" alt="LogoDaEmpresa">
            </a>
            <p class="col-sm-12 text-center"><strong>Salário: R$${
              vagas[vl].salario
            }</strong></p>
          </div>
          <div class="col-sm-8">
            <div class="row ">
              <a href='./vaga.html?id=${vagas[vl].id}' class="text-dark">
                <p id="vaga-nome" class="col-sm-12 text-left text-black mt-2"><strong>${
                  vagas[vl].vaga_nome
                    ? vagas[vl].vaga_nome
                    : JSON.parse(localStorage.getItem('Cargos')).find(
                        (obj) => obj.id === vagas[vl].cargo_id,
                      ).nome
                }</strong></p>
              </a>
              <p class="col-sm-12 text-left"><a href="./cargo.html?id=${
                cargo.id
              }">${cargo.nome}</a> - ${senioridade.nome}</p>
              <a href='./vaga.html?id=${vagas[vl].id}' class="text-dark">
              <p id="descricao" class="col-sm-12 text-left mb-0">${reduzirTexto(
                vagas[vl].descricao,
                100,
              )}</p>
              </a>
              <hr class="col-sm-12">
              <p id="empresa-data" class="col-sm-12 text-left"><a href='./empresa.html?id=${
                vagas[vl].empresa_id
              }'>${empresa.nome}</a> - <small>De: ${dataSimplificada(
        vagas[vl].data_cadastro,
      )} - Até: ${dataSimplificada(vagas[vl].data_exp)}</small></p>
...
(function () {
  ...
  if (!empresa) {
    $('#navBar').append(
      '<a class="nav-link" href="./login.html">Sou empresa</a>',
    );
  } else {
    $('#navBar').append(`
    <li class="nav-item">        
    ...
    <li class="nav-item">        
      ...
      })();" class="nav-link btn btn-link" id="logoff">Desconectar</button>
    </li>
    `);
  }
})();
